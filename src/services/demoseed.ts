...
export const DEMO_CR_SEED = { name: 'Priya (CR)', email: 'cr.demo@example.com' }
...
/** CR accounts can't be created directly via signUp (only a teacher can promote someone) —
 *  so for the demo, we create them as a student first and then promote, same as the real flow. */
async function ensureCrAccount(name: string, email: string): Promise<UserProfile> {
  ...
  const promoted: UserProfile = { ...base, role: 'cr', assignedCourseIds: base.enrolledCourseIds ?? [] }
...
export const DEMO_LOGINS = [
  ...DEMO_STUDENT_SEEDS.map((s) => ({ ...s, role: 'student' as const })),
  ...DEMO_TEACHER_SEEDS.map((t) => ({ ...t, role: 'teacher' as const })),
  { ...DEMO_CR_SEED, role: 'cr' as const },
]