...
export default function MarkPresence({ courses, onSaved }: { courses: Course[]; onSaved: () => void }) {
  ...
  return (
    <div className="card-tint tint-sage max-w-md">
      <h3 className="font-extrabold text-lg mb-2 text-ink">📍 Mark My Presence</h3>
      ...
      {step === 'camera' && (
        <div className="space-y-3">
          {cam.error && <p className="text-sm font-semibold text-[#C0392B]">{cam.error}</p>}
          <video ref={cam.videoRef} className="w-full rounded-2xl border-2 border-ink bg-ink aspect-video" muted playsInline />
      ...
      {step === 'review' && photo && (
        <div className="space-y-3">
          <img src={photo} alt="Captured preview" className="w-full rounded-2xl border-2 border-ink" />
          <div className="flex gap-2">
      ...
      {step === 'done' && result && (
        <div className="space-y-2 text-sm text-ink">
          <p className="font-extrabold text-base">✅ Attendance marked!</p>
          ...
          {result.geoTag && (
            <>
              <p>
                Exact coordinates: {result.geoTag.latitude.toFixed(6)}, {result.geoTag.longitude.toFixed(6)}
              </p>
              {result.geoTag.accuracy && (
                <p>Accuracy: ±{Math.round(result.geoTag.accuracy)} m</p>
              )}
              <a
                href={`https://www.google.com/maps?q=${result.geoTag.latitude},${result.geoTag.longitude}`}
                target="_blank"
                rel="noreferrer"
