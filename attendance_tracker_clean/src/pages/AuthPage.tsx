...
export default function AuthPage() {
  ...
  async function handleDemoLogin(demoEmail: string, demoRole: Role) {
    ...
    try {
      ...
      nav(demoRole === 'teacher' ? '/teacher' : '/student')
    } catch (err: any) {
        ...
        {!demoReady ? (
          <>
            <p className="text-sm text-ink/70 mb-3">
              One-time setup: creates demo student and teacher accounts and sample attendance data in your
              Firebase project. A teacher can then promote any student to class rep from the class overview.
            </p>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div>
                ...
                {DEMO_LOGINS.filter((u) => u.role === 'student').map((u) => (
                  ...
                  >
                    {u.name}
                    {u.isClassRepresentative ? ' • Class rep' : ''}
                  </button>
                ))}
              </div>
              <div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="card-tint tint-sky">
          <div className="flex gap-2 mb-4 bg-cream-soft/70 border-2 border-ink rounded-full p-1">
            <button
              className={`flex-1 py-1.5 rounded-full text-sm font-semibold transition-colors ${mode === 'login' ? 'bg-ink text-cream-soft' : 'text-ink hover:bg-ink/5'
                }`}
            ...
            <button
              className={`flex-1 py-1.5 rounded-full text-sm font-semibold transition-colors ${mode === 'signup' ? 'bg-ink text-cream-soft' : 'text-ink hover:bg-ink/5'
                }`}
