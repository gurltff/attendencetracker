                ...
                <div key={s.uid} className="flex items-center justify-between py-2 text-sm">
                  ...
                  <div className="flex gap-2">
                    <button type="button" disabled={alreadyMarkedToday.has(s.uid)}
                      className={`px-3 py-1 rounded-full text-xs font-semibold border-2 border-ink transition-colors ${marks[s.uid] === 'present' ? 'bg-sage text-ink' : 'bg-transparent text-ink hover:bg-sage/40'
                        }`}
                      onClick={() => setMarks((m) => ({ ...m, [s.uid]: 'present' }))}>
                      Present
                    </button>
                    <button type="button" disabled={alreadyMarkedToday.has(s.uid)}
                      className={`px-3 py-1 rounded-full text-xs font-semibold border-2 border-ink transition-colors ${marks[s.uid] === 'absent' ? 'bg-blush text-ink' : 'bg-transparent text-ink hover:bg-blush/40'
                        }`}
